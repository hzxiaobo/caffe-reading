...
bool NetNeedsUpgrade(const NetParameter& net_param) {
  return NetNeedsV0ToV1Upgrade(net_param) || NetNeedsV1ToV2Upgrade(net_param);
    ...
    //检查读取的网络中，是否包含layer，因为DL里把网络把卷积，ReLu什么的都认为是layer，也就是检查是否包含layer
...
V1LayerParameter_LayerType UpgradeV0LayerType(const string& type) {
  if (type == "accuracy") {
    ...
    //判断输入数据的data层有没有缩放，减平均，crop以及mirror等操作
...
bool UpgradeNetAsNeeded(const string& param_file, NetParameter* param) {
  bool success = true;
  if (NetNeedsV0ToV1Upgrade(*param)) { //函数在本文件中，检查读取的网络中，是否包含layer
  ...
  if (NetNeedsDataUpgrade(*param)) {    //判断输入数据的data层有没有缩放，减平均，crop以及mirror等操作
    LOG(ERROR) << "Attempting to upgrade input file specified using deprecated "
  ...
  if (NetNeedsV1ToV2Upgrade(*param)) {     //检查输入的layers的个数是否大于0
    LOG(ERROR) << "Attempting to upgrade input file specified using deprecated "
...
void ReadNetParamsFromTextFileOrDie(const string& param_file,
  ...
  UpgradeNetAsNeeded(param_file, param); //函数在本cpp里
}

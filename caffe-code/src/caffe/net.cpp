...
Net<Dtype>::Net(const string& param_file, Phase phase) {
  NetParameter param; //不用纠结于NetParameter，因为他是在caffe.proto里写的，使用google protocol buffer来编译和书写的（可以节省大量个代码，详见云笔记）
  ReadNetParamsFromTextFileOrDie(param_file, &param); //函数在upgrade_proto.cpp里，
  param.mutable_state()->set_phase(phase);
  Init(param); //使用读取的NetParameter来初始化网络
}
...
void Net<Dtype>::CopyTrainedLayersFrom(const string trained_filename) {
  NetParameter param;
  ReadNetParamsFromBinaryFileOrDie(trained_filename, &param); //函数在upgrade_proto中，其目的就是读取文件，将其Load在NetParameter中
  CopyTrainedLayersFrom(param); //拿到了NetParameter之后，再调用这个函数将Net初始化
}
